...
export default function Sidebar({ onLogin }) {
  ...
  // MessageList loads messages + artifacts when selectedConversation changes
      ...
      `}>
        <div className="flex flex-col h-full">
          <div className="flex items-center gap-2 px-3.5 py-3.5 border-b border-white/[0.06]">
            ...
            <BrandLogo size="sm" withName className="flex-1 min-w-0" />

            <span className="text-[10px] font-medium text-cyan-300/90 bg-cyan-500/10 border border-cyan-500/20 px-2 py-0.5 rounded-full tracking-wide capitalize shrink-0">
              {userData?.plan ?? "free"}
            </span>
          ...
          <div className="px-4 pt-4 pb-1">
            <button
              onClick={handleCreateConversation}
              className="w-full flex items-center justify-center gap-2 text-sm font-medium text-white bg-gradient-to-br from-cyan-400/90 via-indigo-500 to-violet-600 rounded-xl py-[10px] border border-white/10 cursor-pointer hover:opacity-90 transition-opacity duration-150 shadow-md shadow-indigo-500/20"
            >
              <Plus size={15} />
              New Chat
            </button>
            ...
            {userData ? (
              <div className="flex items-center gap-2.5 cursor-pointer rounded-xl px-3 py-2.5 hover:bg-white/[0.05] transition-colors duration-150">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-slate-600 mt-px capitalize">{userData.plan || "free"}</p>
                </div>
