...
export default function MessageList({ onSuggestion }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      {messages.length === 0 && !isLoading ? (
        <div className="h-full flex flex-col items-center justify-center gap-5 text-center brand-glow rounded-2xl">
          <BrandLogo size="lg" />
          <div className="flex flex-col gap-2">
            <h1 className="text-[22px] font-semibold text-slate-100 tracking-tight">{APP_NAME}</h1>
            <h3 className="text-[15px] font-medium text-slate-400 tracking-tight">{APP_TAGLINE}</h3>
            <p className="text-[13px] text-slate-600 max-w-[280px] leading-relaxed mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-1">
            {["Write a Netflix clone", "Explain Redis", "Build a dashboard"].map((s) => (
              <button
                ...
                className="text-[12px] text-slate-400 bg-white/[0.04] border border-white/[0.08] px-3.5 py-2 rounded-xl hover:bg-white/[0.08] hover:text-slate-200 hover:border-indigo-500/30 transition-all duration-150 cursor-pointer"
              >
                {s}
              </button>
            ))}
